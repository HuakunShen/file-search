...
//!
//! The crawler only walks roots supplied by the caller and applies `kuntu-core`
...
use kuntu_core::{EntryKind, PathPolicy, PolicyDecision, SearchConfig, SearchQuery};
...
mod tests {
  ...
  use kuntu_core::{SearchConfig, SearchRoot};
  ...
  fn temp_dir(name: &str) -> PathBuf {
    let nonce = SystemTime::now()
      ...
      .unwrap()
      .as_nanos();
    std::env::temp_dir().join(format!("kuntu-crawler-{name}-{nonce}"))
  }
