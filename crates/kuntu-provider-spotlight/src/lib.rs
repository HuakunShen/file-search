...
//! sensitive-path denial, dedupe, and ranking remain in `kuntu-core`.
...
mod tests {
  use super::*;
  use kuntu_core::{SearchConfig, SearchQuery, SearchRoot};

  #[test]
  fn build_mdfind_args_scopes_each_enabled_root_and_uses_filename_query() {
    let config = SearchConfig {
      roots: vec![
        // Platform-native fixture roots (see kuntu-core policy.rs#fixture_root):
